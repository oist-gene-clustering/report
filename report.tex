...
When tested on a dataset, the algorithm returns inconsistent results.\\ %%TODO results
...
%% TODO plots pour les copulas et relation entre variables et latent variables
...
Thus this function gives more weight to results from datasets on which the tested algorithms give their best outputs (that is, datasets having the highest mean ARI value across all algorithms), because these datasets are considered more relevant than other datasets where the clustering pattern is less easy to find, and/or may not exist. To my mind, a "good" clustering algorithm should -almost- always be able to find the pattern whenever it exists, and is allowed to give a random, unstable result (i.e., that may be different from one iteration to another) on noise datasets for instance.
...
%I want to show that the parametrization using regular arguments $r$ (the number of times needed to get a success event) and $p$ (the probability of a success event) is equivalent to the parametrization using the mean value obtained $\mu$ and the dispersion parameter $\delta$

%\begin{lemma}[Equivalence between parametrizations]{
...
%\begin{lemma}[Distributions for variables $(Y_i)_i$ knowing the distributions for variables $(X_i)_i \geq 0$]{
%$\forall i,$ if $Y_i \thicksim -X_i$:\\
%\begin{center} $\forall x \leq 0$, \emph{Prob}$(Y_i = x)$ = \emph{Prob}$(X_i = -x)$ \end{center}
%\begin{center} $\forall p \in [0, 1]$, sup\{$x \leq 0$ | \emph{Prob}$(Y_i \leq x) \geq p$\} = sup\{$x \leq 0$ | \emph{Prob}$(X_i \geq -x) \geq p$\} %= inf\{$x \geq 0$ | \emph{Prob}$(X_i \geq x) \geq p$\} \end{center}
%\begin{center} \end{center}
%}\end{lemma}
...
Enfin, j'ai conçu un modèle de l'expression de gène dans une seule cellule, qui tient compte des particularités qui ont été observées dans les données issues de \textit{single-cell RNA sequencing}. J'en ai dérivé une méthode de \textit{clustering} qui est plus spéficiquement orientée vers le clustering de \textit{single-cell RNA sequencing}.\\
 
\textbf{Les arguments en faveur de sa validité}\\

\begin{itemize}
\item Le code d'une première version fonctionnelle de l'application est en ligne, et l'application permet d'intégrer facilement de nouvelles données;
\item L'analyse comparative de plusieurs algorithmes, sur différents ensembles de données, a été réalisée, en prenant en compte la variabilité de la qualité des ensembles de données de test, et permet de distinguer des algorithmes performants au niveau du résultat retourné, du temps de calcul, et de la stabilité du résultat;
\item Le modèle d'expression de gène proposé permet de s'affranchir de l'hypothèse d'indépendance, ou de dépendance uniquement par paires de gènes, dans les niveaux d'expression de gène entre les différents gènes, contrairement à la plupart des méthodes utilisées auparavant, et de souligner les liens de régulation entre les gènes. Une méthode de \textit{clustering} a été implémentée, mais n'est pas fonctionnelle. Cependant, sa conception a permis de distinguer les points importants à respecter pour pouvoir interpréter correctement les résultats d'un algorithme de \textit{clustering} appliqué à des données de séquençage ARN.
\end{itemize}

\textbf{Le bilan et les perspectives}\\

La méthode de séquençage utilisée, \textit{single-cell RNA sequencing}, est encore récente, et de nouvelles améliorations et innovations sont en cours. En particulier, la taille croissante des données proposera à l'avenir de nouveaux défis à relever pour les bioinformaticiens en matière de \textit{clustering}, en particulier au niveau de la complexité temporelle. De plus, la méthode proposée tente de donner un sens au \textit{clustering} retourné, en utilisant la notion de motif d'expression, et en tentant de simuler les réseaux de régulation de gènes.\\

Beaucoup de points dans le mécanisme de régulation de gènes, et dans la génération de sous-populations de cellules, sont encore à éclaircir, et une visualisation claire et intuitive des données est un premier pas vers la compréhension de ces phénomènes.\\
